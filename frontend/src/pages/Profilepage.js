    ...
    <div className="min-h-screen bg-[#FCFAED]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="mb-8">
          <span className="text-xs font-bold tracking-[0.2em] uppercase text-[var(--secondary-color)]">Mon compte</span>
          <h1 className="mt-1 text-3xl sm:text-4xl font-black text-gray-900" style={{ fontFamily: "'Georgia', serif" }}>
            Mon <span className="text-[var(--primary-color)]">Profil</span>
          </h1>
        </motion.div>
              ...
              <div className="px-6 pb-6 -mt-10">
                ...
                {isAdmin && (
                  <span className="inline-flex items-center gap-1.5 mt-3 text-xs font-bold px-3 py-1.5 rounded-full bg-amber-100 text-amber-700">
                    <FaShieldAlt size={10} /> Administrateur
                  </span>
                )}
              </div>
            </div>
          ...
          >
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-lg font-black text-gray-800">Informations personnelles</h3>
                  <p className="text-xs text-gray-500 mt-0.5">Gérez vos informations de compte</p>
                </div>
                {!editing ? (
                  <button
                    onClick={() => setEditing(true)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {/* Prénom */}
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-2 uppercase tracking-wide">Prénom</label>
                  {editing ? (
                    <>
                      <input
                ...
                {/* Nom */}
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-2 uppercase tracking-wide">Nom</label>
                  {editing ? (
                    <>
                      <input
                        value={form.lastname}
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-gray-600 mb-2 uppercase tracking-wide">Adresse email</label>
                  {editing ? (
                    <>
                      <input
                        value={form.email}
            ...
            {/* Card infos compte */}
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8">
              <h3 className="text-lg font-black text-gray-800 mb-5">Détails du compte</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  { label: "Rôle",        value: isAdmin ? "Administrateur" : "Client",                 color: isAdmin ? "text-amber-600 bg-amber-50 border-amber-200" : "text-blue-600 bg-blue-50 border-blue-200" },
                  { label: "Connexion",   value: user.provider === "google" ? "Google" : "Email / Mot de passe", color: "text-gray-700 bg-gray-50 border-gray-200" },
                  { label: "Membre depuis", value: user.created_at ? new Date(user.created_at).toLocaleDateString("fr-FR", { month: "long", year: "numeric" }) : "—", color: "text-gray-700 bg-gray-50 border-gray-200" },
            ...
            {user.provider !== "google" && (
              <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8">
                <h3 className="text-lg font-black text-gray-800 mb-2">Sécurité</h3>
                <p className="text-sm text-gray-500 mb-5">Gérez votre mot de passe et la sécurité du compte</p>
                <button
                ...
                >
                  <FaKey size={13} /> Changer le mot de passe
                </button>
              </div>
            )}
          </motion.div>
